...
<div>
    @php($uniqid = uniqid())
    <img src="{{  url($user->photo) ? $user->avatar_url : '' }}" alt="{{ $user->name }}"
        data-popover-target="popover-user-{{ $user->id }}-{{ $uniqid }}"
        class="w-6 h-6 rounded-full bg-gray-200 bg-cover bg-center" />

    <div data-popover id="popover-user-{{ $user->id }}-{{ $uniqid }}" role="tooltip" class="inline-block absolute invisible z-10 w-64 text-sm font-light text-gray-500
        ...
        <div class="p-3">
            <div class="flex justify-between items-center mb-2">
                <img class="w-10 h-10 rounded-full" src="{{  url($user->photo) ? $user->avatar_url : '' }}"
                    alt="{{ $user->name }}">
            </div>
